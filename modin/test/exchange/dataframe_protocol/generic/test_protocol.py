...
"""Dataframe exchange protocol tests that are common for every execution backend."""
...
from .utils import assert_protocol_dataframe_equal


def test_int_dtype():
    modin_df = pd.DataFrame(test_data["int_data"])
    modin_dfx = modin_df.__dataframe__()
    assert_protocol_dataframe_equal(modin_df, modin_dfx)
    df_equals(modin_df, from_dataframe(modin_dfx))


def test_float_dtype():
    modin_df = pd.DataFrame(test_data["float_data"])
    modin_dfx = modin_df.__dataframe__()
    assert_protocol_dataframe_equal(modin_df, modin_dfx)
    df_equals(modin_df, from_dataframe(modin_dfx))
...
def test_categorical_dtype_two_columns(data, num_partitions):
    ...
    assert tuple(col.describe_categorical.values()) == (
        data[1],
        True,
        {4: "s", 2: "d", 3: "e", 1: "t"},
    )
