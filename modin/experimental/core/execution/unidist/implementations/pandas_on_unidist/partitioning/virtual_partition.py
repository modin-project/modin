...
class PandasOnUnidistDataframeVirtualPartition(PandasDataframeAxisPartition):
    ...
    Parameters
    ----------
    list_of_blocks : Union[list, PandasOnUnidistDataframePartition]
    ...
    # instance_type = unidist.ObjectRef # TODO[unidist]: unidist doesn't provide a public object ObjectRef
    axis = None

    def __init__(self, list_of_blocks, get_ip=False, full_axis=True, call_queue=None):
        if isinstance(list_of_blocks, PandasOnUnidistDataframePartition):
        ...
        if not any(
            isinstance(o, PandasOnUnidistDataframeVirtualPartition)
        ...
        assert (
            len(
                set(
                    o.axis
                    for o in list_of_blocks
                    if isinstance(o, PandasOnUnidistDataframeVirtualPartition)
                )
            )
            == 1
        )
        ...
        if (
            ...
            for o in list_of_blocks:
                new_list_of_blocks.extend(
                    ...
                    o, PandasOnUnidistDataframeVirtualPartition
    ...
    def deploy_axis_func(
        ...
        *partitions : iterable
        ...
        max_retries = kwargs.pop(
            "max_retries", None
        )  # TODO[unidist]: invesigate possibility of usage
    ...
    def drain_call_queue(self):
        ...
        drained = super(PandasOnUnidistDataframeVirtualPartition, self).apply(drain)
...
@unidist.remote
def deploy_unidist_func(func, *args):  # pragma: no cover
    """
