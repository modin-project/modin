...
"""The module defines interface for an axis partition with PyArrow storage format and unidist engine."""

from modin.core.dataframe.pandas.partitioning.axis_partition import (
    BaseDataframeAxisPartition,
)
from .partition import PyarrowOnUnidistDataframePartition

import unidist
import pyarrow


class PyarrowOnUnidistDataframeAxisPartition(BaseDataframeAxisPartition):
    """
    Class defines axis partition interface with PyArrow storage format and unidist engine.
    ...
    def __init__(self, list_of_blocks):
        # Unwrap from PyarrowOnUnidistDataframePartition object for ease of use
    ...
    def apply(self, func, num_splits=None, other_axis_partition=None, **kwargs):
        ...
        other_axis_partition : PyarrowOnUnidistDataframeAxisPartition, optional
            Another ``PyarrowOnUnidistDataframeAxisPartition`` object to apply to
            `func` with this one.
        **kwargs : dict
        ...
        -------
        list
            List with ``PyarrowOnUnidistDataframePartition`` objects.
        ...
        if other_axis_partition is not None:
            return [
                PyarrowOnUnidistDataframePartition(obj)
...
class PyarrowOnUnidistDataframeColumnPartition(PyarrowOnUnidistDataframeAxisPartition):
    """
    The column partition implementation for PyArrow storage format and unidist engine.

    All of the implementation for this class is in the ``PyarrowOnUnidistDataframeAxisPartition``
...
class PyarrowOnUnidistDataframeRowPartition(PyarrowOnUnidistDataframeAxisPartition):
    """
    The row partition implementation for PyArrow storage format and unidist engine.
...
def concat_arrow_table_partitions(axis, partitions):
    ...
    """
    if axis == 0:
        table = pyarrow.unidist.from_batches(
...
def deploy_remote_axis_func(axis, func, num_splits, kwargs, *partitions):
    """
    Deploy a function along a full axis in unidist.
...
def deploy_remote_func_between_two_axis_partitions(
    ...
    """
    Deploy a function along a full axis between two data sets in unidist.
