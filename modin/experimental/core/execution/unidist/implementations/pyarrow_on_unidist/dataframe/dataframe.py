...
"""
Module contains class ``PyarrowOnUnidistDataframe``.

``PyarrowOnUnidistDataframe`` is a dataframe class with PyArrow storage format and unidist engine.
"""

import pandas
from pandas.core.dtypes.cast import find_common_type

from ..partitioning.partition_manager import PyarrowOnUnidistDataframePartitionManager
from modin.core.dataframe.pandas.dataframe.dataframe import PandasDataframe

import unidist


class PyarrowOnUnidistDataframe(PandasDataframe):
    """
    Class for dataframes with PyArrow storage format and unidist engine.

    ``PyarrowOnUnidistDataframe`` implements interfaces specific for PyArrow and unidist,
    ...
    _partition_mgr_cls = PyarrowOnUnidistDataframePartitionManager
    ...
    def combine_dtypes(cls, list_of_dtypes, column_names):
        ...
        """
        dtypes = (
            pandas.concat(unidist.get(list_of_dtypes), axis=1)
    ...
    def to_pandas(self):
        ...
        pandas.DataFrame
        """
        df = super(PyarrowOnUnidistDataframe, self).to_pandas()
        df.index = self.index
