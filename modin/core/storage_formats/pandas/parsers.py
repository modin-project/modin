...
@doc(_doc_pandas_parser_class, data_type="multiple JSON files simultaneously")
class PandasJsonGlobParser(PandasJSONParser):
    @staticmethod
    @doc(_doc_parse_func, parameters=_doc_parse_parameters_common)
    def parse(fname, **kwargs):
        use_json_parser = kwargs.pop("use_json_parser", None)
        if use_json_parser:
            return PandasJSONParser.parse(fname, **kwargs)
