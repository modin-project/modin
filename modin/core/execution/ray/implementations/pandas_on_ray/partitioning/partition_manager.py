...
class PandasOnRayDataframePartitionManager(GenericRayDataframePartitionManager):
    ...
    @remote_function
    def _remote_get_index(*dfs):  # pragma: no cover  # noqa: GL08
        return [df.index for df in dfs]

    @remote_function
    def _remote_get_columns(*dfs):  # pragma: no cover  # noqa: GL08
    ...
    def get_indices(cls, axis, partitions, index_func=None):
        ...
        if index_func is None:
            remote_fn = cls._remote_get_index if axis == 0 else cls._remote_get_columns
