...
class BasePandasDataset(BasePandasDatasetCompat):
    ...
    def __getitem__(self, key):
        ...
        BasePandasDataset
            Located dataset.
        """
        if not self._query_compiler.lazy_execution and self._len() == 0:
    ...
    def _len(self):
        """
        Return length of info axis if it's materialized.
    ...
    def __getattribute__(self, item):
        ...
        if item not in _DEFAULT_BEHAVIOUR and not self._query_compiler.lazy_execution:
            ...
            if callable(attr) and self._empty and hasattr(self._pandas_class, item):
