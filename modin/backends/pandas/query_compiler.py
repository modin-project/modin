...
class PandasQueryCompiler(BaseQueryCompiler):
    ...
    def where(self, cond, other, **kwargs):
        ...
        if isinstance(other, type(self)):
            ...
            first_pass = cond._modin_frame._binary_op(
                where_builder_first_pass, other._modin_frame, join_type="left", name="where stage 1/2"
            )
            ...
            new_modin_frame = self._modin_frame._binary_op(
                where_builder_second_pass, first_pass, join_type="left", name="where stage 2/2"
            )
        ...
        # dataframe.
            ...
            new_modin_frame = self._modin_frame._binary_op(
                where_builder_series, cond._modin_frame, join_type="left", name="where"
            )
        return self.__constructor__(new_modin_frame)
    ...
    def merge(self, right, **kwargs):
        ...
        if how in ["left", "inner"] and left_index is False and right_index is False:
            ...
            new_self = self.__constructor__(
                self._modin_frame._apply_full_axis(1, map_func, name="merge")
            )
            is_reset_index = True
    ...
    count = MapReduceFunction.register(pandas.DataFrame.count, pandas.DataFrame.sum, map_name="count stage 1/2", reduce_name="count stage 2/2")
    max = MapReduceFunction.register(pandas.DataFrame.max, pandas.DataFrame.max, map_name="max stage 1/2", reduce_name="max stage 2/2")
    min = MapReduceFunction.register(pandas.DataFrame.min, pandas.DataFrame.min, map_name="min stage 1/2", reduce_name="min stage 2/2")
    sum = MapReduceFunction.register(pandas.DataFrame.sum, pandas.DataFrame.sum, map_name="sum stage 1/2", reduce_name="sum stage 2/2")
    prod = MapReduceFunction.register(pandas.DataFrame.prod, pandas.DataFrame.prod, map_name="prod stage 1/2", reduce_name="prod stage 2/2")
    any = MapReduceFunction.register(pandas.DataFrame.any, pandas.DataFrame.any, map_name="any stage 1/2", reduce_name="any stage 2/2")
    all = MapReduceFunction.register(pandas.DataFrame.all, pandas.DataFrame.all, map_name="all stage 1/2", reduce_name="all stage 2/2")
    memory_usage = MapReduceFunction.register(
        pandas.DataFrame.memory_usage,
        lambda x, *args, **kwargs: pandas.DataFrame.sum(x),
        axis=0,
        map_name="memory_usage 1/2",
        reduce_name="memory_usage 2/2"
    )
    mean = MapReduceFunction.register(
        ...
        map_name="mean stage 1/2", reduce_name="mean stage 2/2"
    )
    ...
    median = ReductionFunction.register(pandas.DataFrame.median, name="median()")
    ...
    std = ReductionFunction.register(pandas.DataFrame.std, name="std()")
    ...
    def first_valid_index(self):
        ...
        first_result = (
            self.__constructor__(
                self._modin_frame._fold_reduce(0, first_valid_index_builder, name="first_valid_index")
            )
            .min(axis=1)
            .to_pandas()
            .squeeze()
    ...
    def last_valid_index(self):
        ...
        first_result = (
            self.__constructor__(
                self._modin_frame._fold_reduce(0, last_valid_index_builder, name="last_valid_index")
            )
            .max(axis=1)
            .to_pandas()
            .squeeze()
    ...
    def clip(self, lower, upper, **kwargs):
        ...
        if is_list_like(lower) or is_list_like(upper):
            ...
            new_modin_frame = self._modin_frame._map(lambda df: df.clip(**kwargs), name = "clip")
        return self.__constructor__(new_modin_frame)
    ...
    def dot(self, other, squeeze_self=None, squeeze_other=None):
        ...
        new_modin_frame = self._modin_frame._apply_full_axis(
            axis, map_func, new_index=new_index, new_columns=new_columns, name="dot"
        )
        return self.__constructor__(new_modin_frame)
    ...
    def nsort(self, n, columns=None, keep="first", sort_type="nsmallest"):
        ...
        new_modin_frame = self._modin_frame._apply_full_axis(
            axis=0, func=map_func, new_columns=new_columns, name=sort_type
        )
        return self.__constructor__(new_modin_frame)
    ...
    def mode(self, **kwargs):
        ...
        new_modin_frame = self._modin_frame._apply_full_axis(
            axis, mode_builder, new_index=new_index, new_columns=new_columns, name="mode"
        )
    ...
    def fillna(self, **kwargs):
        ...
        if full_axis:
            new_modin_frame = self._modin_frame._fold(axis, fillna, name="fillna")
        else:
    ...
    def query(self, expr, **kwargs):
        ...
        return self.__constructor__(
            self._modin_frame.filter_full_axis(1, query_builder, name="query")
        )
    ...
    def _apply_text_func_elementwise(self, func, axis, *args, **kwargs):
        ...
        kwargs["axis"] = axis
        new_modin_frame = self._modin_frame._apply_full_axis(
            axis, lambda df: getattr(df, func)(**kwargs), name=func
        )
        return self.__constructor__(new_modin_frame)
    ...
    def _callable_func(self, func, axis, *args, **kwargs):
        ...
        if isinstance(pandas.DataFrame().apply(func), pandas.Series):
            new_modin_frame = self._modin_frame._fold_reduce(
                axis, lambda df: df.apply(func, axis=axis, *args, **kwargs), name=func.__name__
            )
        else:
            new_modin_frame = self._modin_frame._apply_full_axis(
                axis, lambda df: df.apply(func, axis=axis, *args, **kwargs), name=func.__name__
