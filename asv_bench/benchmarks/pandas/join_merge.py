...
class Join:
    ...
    def time_join_dataframe_index_shuffle_key_bigger_sort(self, sort):
        execute(self.df_shuf.join(self.df_key2, on="key2", sort=sort))


class JoinNonUnique:
    # outer join of non-unique
    def setup(self):
        date_index = IMPL.date_range("01-Jan-2013", "23-Jan-2013", freq="T")
        ...
        self.fracofday = IMPL.Series(self.fracofday, daily_dates)
        index = IMPL.date_range(date_index.min(), date_index.max(), freq="D")
        self.temp = IMPL.Series(1.0, index)[self.fracofday.index]

    def time_join_non_unique_equal(self):
        execute(self.fracofday * self.temp)
...
class Merge:
    ...
    def time_merge_2intkey(self, sort):
        execute(IMPL.merge(self.left, self.right, sort=sort))

    def time_merge_dataframe_integer_2key(self, sort):
        execute(IMPL.merge(self.df, self.df3, sort=sort))

    def time_merge_dataframe_integer_key(self, sort):
        execute(IMPL.merge(self.df, self.df2, on="key1", sort=sort))

    def time_merge_dataframe_empty_right(self, sort):
        execute(IMPL.merge(self.left, self.right.iloc[:0], sort=sort))

    def time_merge_dataframe_empty_left(self, sort):
        execute(IMPL.merge(self.left.iloc[:0], self.right, sort=sort))
...
class MergeCategoricals:
    def setup(self):
        self.left_object = IMPL.DataFrame(
            {
        ...
        self.right_object = IMPL.DataFrame(
            {
                "X": np.random.choice(range(0, 10), size=(10000,)),
        ...
        execute(self.left_cat)
        self.right_cat = self.right_object.assign(
            Z=self.right_object["Z"].astype("category")
        )
        execute(self.right_cat)

    def time_merge_object(self):
        execute(IMPL.merge(self.left_object, self.right_object, on="X"))

    def time_merge_cat(self):
        execute(IMPL.merge(self.left_cat, self.right_cat, on="X"))
