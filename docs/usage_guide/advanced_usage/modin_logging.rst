...
In the next example, we add logging for not only internal Modin API calls, but also for partition metadata, remote execution (if using Ray), and memory profiling.
...
**Note:**: Logging should be enabled before any Modin calls, and logging of Ray remote execution calls cannot be disabled once enabled.
