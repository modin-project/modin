...
The public API exposed by the children of this class is used in ``BasePandasFrameManager``.

The objects wrapped by the child classes are treated as immutable by ``BasePandasFrameManager`` subclasses
and no logic for updating inplace.

Public API
----------

.. autoclass:: modin.engines.base.frame.partition.BasePandasFramePartition
  :members:
