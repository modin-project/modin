...
The ``BasePandasFrameManager`` object that controls these objects (through the API exposed here) has an invariant
that requires that this object is never returned from a function. It assumes that there will always be
``BasePandasFramePartition`` object stored and structures itself accordingly.
...
Subclasses must implement ``list_of_blocks`` which represents data wrapped by the ``BasePandasFramePartition``
